...
2. In the CMS, switch to the **workspace** that holds the dataset you want to view
   (top-left workspace switcher).

3. Generate a **delivery-scope** API Key in that workspace:
   - Go to `http://localhost:3001/#integrate/api-keys`
   - Click "Generate New Key" → scope: delivery

   The key will be bound to the current workspace — it can only see datasets
   from that workspace.
